...
class LoginApp:
    def __init__(self, root, on_login_success):
        ...
        tk.Label(frame, text="Sistema de Gestión POS", font=("Segoe UI", 12), bg=COL_BG_FRAME, fg="#7F8C8D").pack(pady=(0, 40))
        ...
        tk.Label(frame, text="Contraseña", font=FONT_BOLD, bg=COL_BG_FRAME, fg=COL_TEXT_DARK).pack(anchor="w", pady=(20, 0))
        self.entry_pass = ttk.Entry(frame, font=("Segoe UI", 14), show="●")
        self.entry_pass.pack(fill="x", pady=10)
        self.entry_pass.bind("<Return>", self.verificar_login)
        
        tk.Button(frame, text="INICIAR SESIÓN", bg=COL_PRIMARY, fg=COL_TEXT_LIGHT, font=("Segoe UI", 12, "bold"), 
...
class SistemaVentasApp:
    def __init__(self, root):
        self.root = root
        self.root.title("POS MARTILLO 3 GOLPES - V10.21 IMPRESIÓN SUMATRA")
        self.root.state('zoomed')
        self.root.configure(bg=COL_BG_MAIN)
        
        # --- VERIFICACIÓN DE MOTOR DE IMPRESIÓN ---
        self.ruta_sumatra = "SumatraPDF.exe" # Debe estar en la misma carpeta
        if not os.path.exists(self.ruta_sumatra):
            messagebox.showwarning("FALTA ARCHIVO DE IMPRESIÓN", 
                "Para imprimir automáticamente sin errores, necesitas descargar 'SumatraPDF.exe' y ponerlo en esta carpeta.\n\n"
                "El sistema funcionará, pero abrirá los PDF manualmente hasta que agregues ese archivo.")
        ...
        self.t_caja = ttk.Frame(tabs, style="Card.TFrame"); tabs.add(self.t_caja, text='CAJA DE VENTAS')
        ...
        self.t_conf = ttk.Frame(tabs, style="Card.TFrame"); tabs.add(self.t_conf, text='CONFIGURACIÓN')
    ...
    def crear_tablas(self):
        ...
        if c.fetchone()[0] == 0:
            c.execute("INSERT INTO config (id, nombre_empresa, rut_empresa, direccion, telefono, correo) VALUES (1, 'FERRETERÍA MARTILLO 3 GOLPES', '77.808.144-k', 'Simón Bolívar 284, Chillán', '995536798', 'Martillo3golpes@gmail.com')")
        self.conn.commit()
    ...
    def ui_conf(self):
        ...
        lbls = ["Nombre Fantasía:", "RUT Empresa:", "Dirección:", "Teléfono:", "Correo Electrónico:"]
        ...
        for i, l in enumerate(lbls):
            ...
            self.e_cfg[keys[i]] = e
        tk.Button(f, text="GUARDAR CONFIGURACIÓN", bg=COL_PRIMARY, fg="white", font=FONT_BOLD, relief="flat", padx=20, pady=10, command=self.save_cfg).grid(row=5, column=1, pady=30)
        self.load_cfg()
    ...
    def save_cfg(self):
        ...
        self.conn.commit(); self.load_cfg(); self.popup_msg("Éxito", "Datos de empresa actualizados correctamente.")

    # ==========================================
    # --- BODEGA ---
    # ==========================================
    def ui_bodega(self):
        f = ttk.LabelFrame(self.t_bodega, text="Gestión de Producto (F6 para Opciones)", padding=15); f.pack(fill="x", padx=20, pady=20)
        ttk.Label(f, text="Código de Barra:", font=FONT_BOLD).grid(row=0, column=0)
        ...
        frame_busq = tk.Frame(self.t_bodega, bg=COL_BG_MAIN, pady=5); frame_busq.pack(fill="x", padx=20)
        tk.Label(frame_busq, text="🔍 BUSCADOR RÁPIDO:", font=FONT_BOLD, bg=COL_BG_MAIN).pack(side="left")
        self.entry_busq_bod = ttk.Entry(frame_busq, font=FONT_MAIN); self.entry_busq_bod.pack(side="left", fill="x", expand=True, padx=10)
        self.entry_busq_bod.bind("<KeyRelease>", self.filtrar_bodega)

        # --- TABLA CON SCROLLBAR ---
        frame_tabla = tk.Frame(self.t_bodega) # Contenedor
        frame_tabla.pack(fill="both", expand=True, padx=20, pady=10)

        scrollbar = ttk.Scrollbar(frame_tabla, orient="vertical")
        
        self.tree_bod = ttk.Treeview(frame_tabla, columns=("C","N","P","CN","CI","V","S"), show="headings", yscrollcommand=scrollbar.set)
        
        scrollbar.config(command=self.tree_bod.yview)
        scrollbar.pack(side="right", fill="y") # Barra a la derecha
        
        self.tree_bod.heading("C", text="CÓDIGO"); self.tree_bod.column("C", width=100)
        ...
        self.tree_bod.pack(side="left", fill="both", expand=True) # Tabla llena el resto
        
        # DEFINICIÓN DEL COLOR ROJO (Para la actualización 2)
        self.tree_bod.tag_configure("sin_stock", background="#E74C3C", foreground="white") # Rojo fuerte
    ...
    def filtrar_bodega(self, event):
        ...
        for r in self.conn.cursor().execute(sql, (f"%{query}%", f"%{query}%")):
            ...
            # Lógica del Rojo aquí también
            mis_tags = (r[0], "sin_stock") if r[5] <= 0 else (r[0],)
            
            self.tree_bod.insert("", tk.END, values=(r[1], r[2], r[3], cn, ci, pv, r[5]), tags=mis_tags)
    ...
    def evento_f6_bodega(self, event):
        ...
        if not tags: return
        prod_id = tags[0]
        cur = self.conn.cursor(); cur.execute("SELECT * FROM productos WHERE id=?", (prod_id,)); p = cur.fetchone() 
        w = Toplevel(self.root); w.geometry("450x300"); w.configure(bg=COL_BG_FRAME); w.transient(self.root)
        x = self.root.winfo_x() + (self.root.winfo_width()/2) - 225; y = self.root.winfo_y() + (self.root.winfo_height()/2) - 150
        w.geometry(f"+{int(x)}+{int(y)}")
        tk.Label(w, text=f"GESTIONAR: {p[2]}", font=FONT_BOLD, bg=COL_BG_FRAME, fg=COL_PRIMARY).pack(pady=15)
        def eliminar():
            if messagebox.askyesno("Confirmar", f"¿Eliminar '{p[2]}' definitivamente?"):
        ...
        def modificar_completo(): w.destroy(); self.abrir_ventana_modificar(prod_id)
        tk.Button(w, text="🗑️ ELIMINAR PRODUCTO", bg=COL_ALERT, fg="white", font=FONT_BOLD, relief="flat", pady=12, width=35, command=eliminar).pack(pady=10)
        tk.Button(w, text="✏️ MODIFICAR PRODUCTO COMPLETO", bg=COL_WARNING, fg="white", font=FONT_BOLD, relief="flat", pady=12, width=35, command=modificar_completo).pack(pady=10)

    def abrir_ventana_modificar(self, pid):
        cur = self.conn.cursor(); cur.execute("SELECT * FROM productos WHERE id=?", (pid,)); p = cur.fetchone() 
        top = Toplevel(self.root); top.geometry("750x400"); top.configure(bg=COL_BG_MAIN); top.transient(self.root)
        x = self.root.winfo_x() + (self.root.winfo_width()/2) - 375; y = self.root.winfo_y() + (self.root.winfo_height()/2) - 200
        top.geometry(f"+{int(x)}+{int(y)}")
        f = tk.Frame(top, bg=COL_BG_FRAME, padx=20, pady=20); f.pack(expand=True, fill="both", padx=10, pady=10)
        tk.Label(f, text="Código:", bg=COL_BG_FRAME).grid(row=0, column=0, sticky="e")
        ...
        e_venta = ttk.Entry(f, width=20, font=FONT_BOLD); e_venta.grid(row=3, column=2); e_venta.insert(0, int(p[4]))
        def cal_iva(e):
        ...
        e_neto.bind("<KeyRelease>", cal_iva); e_civa.bind("<KeyRelease>", cal_net)
        def guardar_cambios():
            try:
                pr = float(e_venta.get()); st = int(e_stk.get())
                cn = float(e_neto.get()) if e_neto.get() else 0; ci = float(e_civa.get()) if e_civa.get() else 0
                ...
                self.conn.commit(); self.load_bod(); top.destroy(); self.popup_msg("Éxito", "Producto actualizado.")
    ...
    def calc_iva_costo(self, e):
        try: val = float(self.e_b_neto.get()); self.e_b_costo_iva.delete(0,tk.END); self.e_b_costo_iva.insert(0, str(int(val * 1.19)))
        except: pass
    def calc_neto_costo(self, e):
        try: val = float(self.e_b_costo_iva.get()); self.e_b_neto.delete(0,tk.END); self.e_b_neto.insert(0, str(int(val / 1.19)))
        except: pass
    def save_prod(self):
        c, n, p = self.e_b_cod.get(), self.e_b_nom.get(), self.e_b_prov.get()
        try: 
            pr_venta = float(self.e_b_venta.get()); st = int(self.e_b_stk.get())
        ...
        except: self.popup_msg("Error", "Revise Precio Venta y Stock", True); return
        cursor = self.conn.cursor()
        cursor.execute("SELECT id, stock FROM productos WHERE codigo=? AND codigo!=''", (c,))
        exist = cursor.fetchone()
        if not exist: cursor.execute("SELECT id, stock FROM productos WHERE nombre=? AND proveedor=?", (n, p)); exist = cursor.fetchone()
        if exist:
            cursor.execute("UPDATE productos SET stock=?, precio_iva=?, nombre=?, proveedor=?, codigo=?, costo_neto=?, costo_iva=? WHERE id=?", (exist[1]+st, pr_venta, n, p, c, c_neto, c_iva, exist[0]))
            ...
            self.popup_msg("Éxito", "Producto nuevo creado.")
    ...
    def load_bod(self):
        ...
        for r in self.conn.cursor().execute("SELECT id, codigo, nombre, proveedor, precio_iva, stock, costo_neto, costo_iva FROM productos"):
            ...
            pv = f"${int(r[4]):,}".replace(",", ".")
                ...
                # Lógica del Rojo: Si stock (r[5]) es menor o igual a 0, usa tag rojo
            mis_tags = (r[0], "sin_stock") if r[5] <= 0 else (r[0],)
    ...
    def ui_cli(self):
        ...
        ttk.Label(f, text="Razón Social:").grid(row=0, column=2)
        self.ec_raz = ttk.Entry(f, width=30); self.ec_raz.grid(row=0, column=3, padx=10)
        ttk.Label(f, text="Dirección:").grid(row=0, column=4)
        ...
        self.tree_cli.heading("R", text="RUT"); self.tree_cli.heading("N", text="RAZÓN SOCIAL"); self.tree_cli.heading("D", text="DIRECCIÓN")
        self.tree_cli.pack(fill="both", expand=True, padx=20, pady=10)
        self.load_cli()
    def fmt_cli_rut(self): r = self.fmt_rut(self.ec_rut.get()); self.ec_rut.delete(0, tk.END); self.ec_rut.insert(0, r)
    def save_cli(self):
    ...
    def ui_caja(self):
        ...
        lf_scan = ttk.LabelFrame(top, text="Escáner"); lf_scan.pack(side="left", padx=10)
        ...
        tk.Button(top, text="🔎 BUSCADOR MANUAL", bg="#F39C12", fg="white", font=FONT_BOLD, relief="flat", padx=15, command=self.open_find).pack(side="left", padx=20, fill="y")
        ...
        ttk.Radiobutton(mid, text="Cotización", variable=self.v_tipo, value="Cotizacion", command=self.toggle_fact, style='TRadiobutton').pack(side="left", padx=20)
        ...
        tk.Label(self.f_cli_vta, text="Dirección:", bg=COL_BG_FRAME, font=FONT_BOLD).pack(side="left")
    ...
    def eliminar_item_carrito(self, event):
        ...
        if index >= 0 and index < len(self.cart): del self.cart[index]; self.upd_cart(); self.popup_msg("Eliminado", "Producto quitado del carrito.")
    def toggle_fact(self):
        if self.v_tipo.get() == "Factura" or self.v_tipo.get() == "Cotizacion": self.f_cli_vta.pack(fill="x", padx=20)
    ...
    def find_cli_vta(self, e):
        ...
        if res: self.ev_nom.delete(0, tk.END); self.ev_nom.insert(0, res[0]); self.ev_dir.delete(0, tk.END); self.ev_dir.insert(0, res[1])
    ...
    def open_find(self):
        ...
        def sel(ev):
            item = t.selection(); 
            if item: v = t.item(item, "values"); tags = t.item(item, "tags"); w.destroy(); self.ask_qty((tags[0], v[0], tags[1], v[2]))
        e.bind("<KeyRelease>", sch); e.bind("<Return>", lambda ev: t.focus()); t.bind("<Return>", sel); t.bind("<Double-1>", sel)
    def ask_qty(self, p):
        stk = int(float(p[3]))
        def add():
            try:
                q = int(eq.get())
                if self.v_tipo.get() != "Cotizacion" and q > stk: self.popup_msg("Stock Insuficiente", f"Solo quedan {stk} unidades.", True); return
                for i in self.cart:
                    if str(i['id']) == str(p[0]): i['cant'] += q; i['tot'] = i['cant'] * float(p[2]); self.upd_cart(); w.destroy(); return
        ...
        wx = self.root.winfo_x() + (self.root.winfo_width()/2) - 150; wy = self.root.winfo_y() + (self.root.winfo_height()/2) - 90
    ...
    def upd_cart(self):
        ...
        for x in self.cart: self.tree_cart.insert("", tk.END, values=(x['nom'], x['cant'], f"${int(x['tot']):,}".replace(",", ".")))
    ...
    def pay(self):
        ...
        if tipo == "Factura" or tipo == "Cotizacion":
            ...
            if not r or not n: self.popup_msg("Error", f"Para {tipo} ingrese RUT y Razón.", True); return
        ...
        if tipo == "Cotizacion": self.popup_msg("Listo", "Cotización generada correctamente (Stock intacto).")

   # ==========================================
    # SECCIÓN DE BOLETA / PDF (DISEÑO RETAIL)
    # ==========================================
    def gen_pdf(self, cli, gt, fecha, tipo):
        # 1. Calcular altura dinámica (Más espacio por producto para que se vea grande)
        alto_extra = len(self.cart) * 30 
        fn = f"doc_{datetime.now().strftime('%H%M%S')}.pdf"
        c = canvas.Canvas(fn, pagesize=(ANCHO_TICKET, ALTO_BASE + alto_extra))
        emp = self.datos_emp
        
        # Coordenada Y inicial (dibujamos de abajo hacia arriba)
        y = ALTO_BASE + alto_extra - 20
        
        # --- LOGO ---
        # --- LOGO (OSCURECIDO / SUPERPUESTO) ---
        if os.path.exists("logo.jpeg"): 
            try: 
                # Se dibuja 3 veces para saturar el color y que se vea "negrita"
                c.drawImage("logo.jpeg", 25*mm, y-25*mm, 30*mm, 30*mm, mask='auto')
                c.drawImage("logo.jpeg", 25*mm, y-25*mm, 30*mm, 30*mm, mask='auto')
                c.drawImage("logo.jpeg", 25*mm, y-25*mm, 30*mm, 30*mm, mask='auto')
                y -= 35*mm
            except: pass
        ...
        c.drawCentredString(ANCHO_TICKET/2, y, emp['nom']); y-=14
        
        c.setFont("Helvetica", 9)
        for l in [f"RUT: {emp['rut']}", emp['dir'], f"Fono: {emp['tel']}", f"Email: {emp['mail']}"]:
            c.drawCentredString(ANCHO_TICKET/2, y, l); y-=11
        
        # Separador
        y -= 5; c.setStrokeColor(colors.black); c.setLineWidth(1)
        c.line(5, y, ANCHO_TICKET-5, y); y -= 15
        
        # --- TÍTULO DEL DOCUMENTO ---
        titulo_doc = "COTIZACIÓN FORMAL" if tipo == "Cotizacion" else tipo.upper()
        c.setFont("Helvetica-Bold", 16) # Título bien grande
        c.drawCentredString(ANCHO_TICKET/2, y, titulo_doc); y -= 18
        
        # --- DATOS DE VENTA ---
        c.setFont("Helvetica", 10)
        c.drawString(5, y, f"Fecha: {fecha}"); y -= 12
        if tipo != "Cotizacion": 
            c.drawString(5, y, f"Pago: {self.cb_pay.get()}"); y -= 12
        
        # Datos Cliente (Si aplica)
        if tipo == "Factura" or tipo == "Cotizacion":
            c.drawString(5, y, "-"*42); y-=12
            c.drawString(5, y, f"Cliente: {cli['razon'][:25]}"); y-=12
            c.drawString(5, y, f"RUT: {cli['rut']}"); y-=12
            c.drawString(5, y, f"Dir: {cli.get('dir','')[:30]}"); y-=15

        y -= 5
        
        # --- ENCABEZADOS DE TABLA (ESTILO NEGRO) ---
        c.setFillColor(colors.black)
        c.rect(2, y-2, ANCHO_TICKET-4, 14, fill=1) # Fondo Negro
        c.setFillColor(colors.white) # Letra Blanca
        c.setFont("Helvetica-Bold", 9)
        c.drawString(5, y+2, "CANT")
        c.drawString(35, y+2, "DESCRIPCIÓN")
        c.drawRightString(ANCHO_TICKET-5, y+2, "TOTAL")
        c.setFillColor(colors.black) # Volver a negro para el resto
        y -= 15
        
        # --- LISTADO DE PRODUCTOS ---
        c.setFont("Helvetica-Bold", 10) # Nombre del producto en Negrita
        
        for i in self.cart:
            # Línea 1: Nombre del producto
            nombre_prod = i['nom'][:35] 
            c.drawString(5, y, nombre_prod)
            y -= 12
            
            # Línea 2: Detalle numérico (Cantidad x Precio)
            c.setFont("Helvetica", 10)
            detalle_nums = f"{i['cant']} x ${int(i['pre']):,}".replace(",", ".")
            c.drawString(15, y, detalle_nums)
            
            # Total del ítem a la derecha
            c.setFont("Helvetica-Bold", 10)
            c.drawRightString(ANCHO_TICKET-5, y, f"${int(i['tot']):,}".replace(",", "."))
            y -= 8
            
            # Línea separadora gris suave
            c.setStrokeColor(colors.lightgrey)
            c.line(5, y, ANCHO_TICKET-5, y)
            y -= 10
            c.setStrokeColor(colors.black) # Reset color línea
            c.setFont("Helvetica-Bold", 10) # Reset fuente para el siguiente nombre
        ...
        c.setFont("Helvetica", 10)
        c.drawRightString(ANCHO_TICKET-5, y, f"NETO: ${neto:,}".replace(",", ".")); y -= 12
        c.drawRightString(ANCHO_TICKET-5, y, f"IVA (19%): ${iva:,}".replace(",", ".")); y -= 18
        
        # TOTAL GIGANTE
        c.setFont("Helvetica-Bold", 18) 
        c.drawRightString(ANCHO_TICKET-5, y, f"TOTAL: ${int(gt):,}".replace(",", "."))
        y -= 25
        
        # --- PIE DE PÁGINA ---
        if tipo == "Cotizacion":
            c.setFont("Helvetica-Oblique", 9)
            c.drawCentredString(ANCHO_TICKET/2, y, "Válido por 15 días / Hasta agotar stock")
        else:
            c.setFont("Helvetica", 9)
            c.drawCentredString(ANCHO_TICKET/2, y, "¡GRACIAS POR PREFERIRNOS!")
            y -= 10
            c.setFont("Helvetica", 7)
            c.drawCentredString(ANCHO_TICKET/2, y, "Copia Cliente")
            
        c.save()
        
        # --- ENVIAR A IMPRESORA ---
        self.imprimir_silencioso(fn)

    def imprimir_silencioso(self, archivo):
        # 1. Intentar con SumatraPDF (La forma más robusta y profesional)
        if os.path.exists(self.ruta_sumatra):
            try:
                # Orden de comando silenciosa
                subprocess.call([self.ruta_sumatra, '-print-to-default', '-silent', archivo], shell=True)
                return # Éxito, salimos
            except Exception as e:
                print(f"Error Sumatra: {e}")
        
        # 2. Si falla o no está Sumatra, abrir PDF normal (Fallback)
        try: os.startfile(archivo, "print")
        except: pass
    ...
    def mostrar_preview_reporte(self):
        h = datetime.now().strftime("%Y-%m-%d"); c = self.conn.cursor()
        c.execute("SELECT SUM(total), COUNT(*) FROM ventas WHERE fecha LIKE ?", (f"{h}%",)); res = c.fetchone(); t = res[0] or 0; cnt = res[1]
        c.execute("SELECT medio_pago, SUM(total) FROM ventas WHERE fecha LIKE ? GROUP BY medio_pago", (f"{h}%",)); d = c.fetchall()
        avg = int(t/cnt) if cnt > 0 else 0; txt = f"VISTA PREVIA - {h}\n\nVENTAS: ${int(t):,}\nTRANS: {cnt}\nPROM: ${avg:,}\n\n".replace(",", ".")
        for m, v in d: txt += f"{m}: ${int(v):,}\n".replace(",", "."); self.txt_rep.delete("1.0", tk.END); self.txt_rep.insert(tk.END, txt)
    def generar_cierre_pdf(self):
        h = datetime.now().strftime("%Y-%m-%d"); ts = datetime.now().strftime("%H:%M:%S"); c = self.conn.cursor()
        c.execute("SELECT SUM(total), COUNT(*) FROM ventas WHERE fecha LIKE ?", (f"{h}%",)); res = c.fetchone(); t = res[0] or 0; cnt = res[1]
        c.execute("SELECT medio_pago, SUM(total) FROM ventas WHERE fecha LIKE ? GROUP BY medio_pago", (f"{h}%",)); d = c.fetchall(); avg = int(t/cnt) if cnt > 0 else 0
        fn = f"Cierre_{datetime.now().strftime('%Y-%m-%d_%H%M%S')}.pdf"; p = canvas.Canvas(fn, pagesize= (210*mm, 297*mm)); width, height = (210*mm, 297*mm)
        p.setFillColor(colors.HexColor("#2980B9")); p.rect(0, height-40*mm, width, 40*mm, fill=True, stroke=False)
        p.setFillColor(colors.white); p.setFont("Helvetica-Bold", 24); p.drawString(20*mm, height-25*mm, "REPORTE DE CIERRE DE CAJA")
        p.setFont("Helvetica", 12); p.drawString(20*mm, height-35*mm, f"Fecha: {h}  |  Hora: {ts}")
        y = height - 70*mm; box_w = 50*mm
        p.setFillColor(colors.HexColor("#ECF0F1")); p.setStrokeColor(colors.gray); p.rect(20*mm, y, box_w, 20*mm, fill=True)
        ...
        y -= 20*mm; p.setFont("Helvetica-Bold", 12); p.drawString(20*mm, y, "DESGLOSE POR MEDIO DE PAGO"); y -= 5*mm
        p.setFillColor(colors.HexColor("#34495E")); p.rect(20*mm, y-8*mm, 170*mm, 8*mm, fill=True)
        p.setFillColor(colors.white); p.setFont("Helvetica-Bold", 10); p.drawString(25*mm, y-6*mm, "MEDIO DE PAGO"); p.drawString(150*mm, y-6*mm, "MONTO"); y -= 15*mm
        p.setFillColor(colors.black); p.setFont("Helvetica", 10)
        for m, v in d:
            medio = m if m else "No especificado"; p.drawString(25*mm, y, medio); p.drawRightString(180*mm, y, f"${int(v):,}".replace(",", "."))
            p.setStrokeColor(colors.lightgrey); p.line(20*mm, y-2*mm, 190*mm, y-2*mm); y -= 10*mm
        p.setFont("Helvetica-Oblique", 8); p.drawCentredString(width/2, 20*mm, "Generado automáticamente por Sistema POS Martillo 3 Golpes")
        p.save(); self.txt_rep.delete("1.0", tk.END); self.popup_msg("Turno Cerrado", f"Se generó el reporte ejecutivo:\n{fn}\n\nVista limpiada.")
